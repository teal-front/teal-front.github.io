/**
 * form: dio
 * 包含： $.localStorage & $.sessionStorage & error_send(window.wx)
 * 估计是财付通内部流出
 */
...
var Module = seajs.Module
  ...
  function setComboHash(uris) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (mod.status < FETCHING && !isExcluded(uri) && !isComboUri(uri) && (ext == '.js' || ext == '.css')) {
    ...
    if (needComboUris.length > 1) {
	  var group = uris2paths(needComboUris);
	  for (var i = 0; i < needComboUris.length; ++i){
		var uri = needComboUris[i], g = getExt(uri);
		comboHash[uri] = group[g];
	  }
  ...
  function setRequestUri(data) {
    var ext = getExt(data.uri)
    if (ext == '.js' || ext == '.css')
        data.requestUri = comboHash[data.uri] || data.uri.replace("/c/=", "")
    else
  ...
  function uris2paths(uris) {
	var base = seajs.data.base, ret = [], group = {};
	
	var comboSyntax = data.comboSyntax || ["??", ","];
	
	for (var i = 0; i < uris.length; ++i){
		var uri = uris[i],
	    	key = uri.substr(base.length),
			g = getExt(uri);
		
		if (group[g]){
			group[g] += comboSyntax[1] + key;
		}else{
			group[g] = base + key;
		}
	}
...
(function(){     
    ...
    $.extend($,{
         ...
         * @example
           ...
           var str = $.format(tpl,record,function(item){  
        ...
        */
        ...
        getParameter: function(name,param,unfilter,undecode){
            var obj = {},tmp,                
                str = $.type(param) == "string" ? param : location.search.replace("?","");
    ...
    var storage = function(){
        ...
        var cacheS = $.isSessionAble ? 
            $.parseJSON(sessionStorage.getItem(STORAGE_KEY)||"{}") : {};
        var cacheL = $.isLocalAble ? 
        ...
        var _setItem = function(key,value,persistence){
            count = MAX_TRY;  
            if(persistence){
                cacheL[key] = {"v" : value, "t": +new Date};
                _flush(persistence);
            }else{
                cacheS[key] = {"v" : value, "t": +new Date};
                _flush(persistence);
            }                
        };
        ...
        var _getItem = function(key,persistence){
            ...
            return $.isPlainObject(v) ? $.extend(true,{},v) : 
    ...
    (function(){
        ...
        var data={
            fakeid: (wx.data && wx.data.uin || ""),
            userAgent:window.navigator.userAgent,
            url:location.href
        };
        
        $("img").error(function(){
            send(1,$.format("[fakeid={fakeid}] [img] [url={url}] [useragent={userAgent}] [src={0}]",$.extend(data,{
                "src" : $(this).attr("src")||""
            })));
        });

        $("script").error(function(){
            send(2,$.format("[fakeid={fakeid}] [script] [url={url}] [useragent={userAgent}] [src={0}]",$.extend(data,{
                "src" : $(this).attr("src")||""
            })));
        });
        ...
        function send(type,msg){
            ...
            if (msg.indexOf("ueditor") != -1 || msg.indexOf("media/appmsg_edit") != -1){
                return ;
            }
            $.ajax({
        ...
        wx.jslog = function(d, e, key){
            key = key ||3;
            d = $.extend(d, data);
            var e_str = [];
            if (!!e){
                $.each(["message", "stack", "lineNumber"], function(k, v){
                    e_str.push(v + ":" + (e[v]||""));
                });
            }
            d.e = e_str.join(';').replace(/\s/g, " ");
            var msg = "[fakeid={fakeid}] [script] [url={url}] [useragent={userAgent}] [src={src}] [exception={e}]".format(d);
            send(key, msg);
...
(function(seajs){     
    ...
    var base = location.hostname == 'action.weixin.qq.com' ? 'https://res.wx.qq.com/' : '/';
    
    var getExt = function(url){
        var p = url.lastIndexOf(".");
        return p >= 0 ? url.substring(p) : "";
    };

    if (typeof MODULES != 'undefined'){
        base += "c/=";        
    }
    var pathinfo = seajs.data.pathinfo = {
        '.js'   : 'paymchres/zh_CN/htmledition/js/',
        '.tpl'  : 'paymchres/zh_CN/htmledition/js/',
        '.html' : 'paymchres/zh_CN/htmledition/js/',
        '.css'  : 'paymchres/zh_CN/htmledition/style/'
    };
    ...
    var mapurl = function(url){
        ...
        var base = seajs.data.base,
            key = url.substr(base.length);   

        if(typeof MODULES != 'undefined' && !!MODULES[key]) {
            var file = MODULES[key], ret = file;
            if (ret.indexOf(base.replace("/c/=", "")) == -1){
                ret = base + file;
            }
        ...
        return base.replace("/c/=", "") + pathinfo[ext] + key + "?20140324";    
    }
